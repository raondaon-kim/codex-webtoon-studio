...
1. `story-bible/story-bible.json`을 먼저 확정하고, 영지가 서사의 핵심이면 `territory/*-profile.json`으로 지리·환경·고질 문제를 개별 재난보다 먼저 정한다.
2. `visual-bible/**`를 확정한다.
...
- `territory_profile`의 재난 노출은 발생 후보일 뿐이다. 특정 재난과 발생 시점은 에피소드 스크립트에서 확정한다.
