...
1. `story-bible/story-bible.json`과 `visual-bible/**`를 먼저 확정한다.
2. 회차별 `script.json`을 작성한다.
3. `scroll-plan.json`에서 세로 리듬, 여백, 리빌을 설계한다.
4. 각 컷을 `briefs/*.json`으로 작성한다.
5. `brief-to-image-prompt` skill과 컴파일러로 `render-tasks/*.json`을 만든다.
6. 사용자의 실행 승인이 있을 때만 이미지 생성기를 `--execute`로 호출한다.
7. 레터링/조립/슬라이싱 후 `qc-report.json`을 생성한다.
